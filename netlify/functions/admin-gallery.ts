...
export const handler: Handler = async (event) => {
  ...
  if (event.httpMethod === 'GET') {
    ...
    try {
      ...
      return {
        ...
        body: JSON.stringify({ galleryItems: data || [] }),
      };
    } catch (err: any) {
      return {
        statusCode: 500,
  ...
  if (event.httpMethod === 'POST') {
    try {
      ...
      const { slug: bodySlug, ...galleryItemData } = body;
      ...
      if (error) {
        return {
          statusCode: 500,
          headers: { 'Access-Control-Allow-Origin': '*' },
          body: JSON.stringify({ error: 'Failed to create gallery item' }),
        };
      }
    ...
    try {
      ...
      if (!id) {
        return {
          statusCode: 400,
          headers: { 'Access-Control-Allow-Origin': '*' },
          body: JSON.stringify({ error: 'Missing gallery item id' }),
        };
      }
      ...
      if (error) {
        return {
          statusCode: 500,
          headers: { 'Access-Control-Allow-Origin': '*' },
          body: JSON.stringify({ error: 'Failed to update gallery item' }),
        };
      }
    ...
    try {
      ...
      if (error) {
        return {
          statusCode: 500,
          headers: { 'Access-Control-Allow-Origin': '*' },
          body: JSON.stringify({ error: 'Failed to delete gallery item' }),
        };
      }
